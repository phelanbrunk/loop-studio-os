          ...
          {!hermes.isRunning ? (
            <button
              ...
              style={{
                ...
                color: '#36CFC9', border: '1px solid rgba(54,207,201,0.4)',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = 'linear-gradient(135deg, rgba(54,207,201,0.3), rgba(77,138,255,0.3))'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'linear-gradient(135deg, rgba(54,207,201,0.2), rgba(77,138,255,0.2))'; }}
            >
              <Play size={13} /> Start
            </button>
          ) : (
            <>
              <button onClick={hermes.pause}
                className="flex-1 flex items-center justify-center gap-1.5 h-8 rounded-lg text-[11px] font-medium transition-all"
                style={{ backgroundColor: 'rgba(245,197,66,0.15)', color: '#F5C542', border: '1px solid rgba(245,197,66,0.3)' }}
                onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'rgba(245,197,66,0.25)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'rgba(245,197,66,0.15)'; }}
              >
                <Pause size={13} /> Pause
              </button>
              <button onClick={hermes.stop}
      ...
      <div className="shrink-0 p-4 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
        ...
        <div className="space-y-1.5">
          {PHASE_CONFIG.map((pc, idx) => {
            ...
            const Icon = pc.icon;
            return (
              <div key={pc.phase} className="flex items-center gap-2 h-6 px-2 rounded-md transition-all"
                style={{ backgroundColor: isActive ? `${pc.color}15` : 'transparent', borderLeft: isActive ? `2px solid ${pc.color}` : '2px solid transparent' }}>
                {isCompleted ? <CheckCircle size={11} style={{ color: '#36CFC9' }} /> : <Icon size={11} style={{ color: isActive ? pc.color : '#5E626A' }} />}
                <span className="text-[11px] flex-1" style={{ color: isActive ? pc.color : isCompleted ? '#A1A4AA' : '#5E626A', fontWeight: isActive ? 600 : 400 }}>
                  {idx + 1}. {pc.label}
                </span>
                {isActive && <div className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: pc.color }} />}
              </div>
            );
          })}
        </div>
              ...
              <div className="p-3 space-y-1.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] font-semibold" style={{ color: '#F5C542' }}>Phelan</span>
                  </div>
                  <button onClick={hermes.clearNotifications} className="text-[9px] px-1.5 py-0.5 rounded" style={{ color: '#5E626A' }}>Clear</button>
                </div>
                {hermes.phelanNotifications.map((notif, i) => (
                  <div key={i} className="flex items-center gap-1.5 p-1.5 rounded" style={{ backgroundColor: 'rgba(245,197,66,0.08)' }}>
        ...
        <div className="p-4">
          ...
          <div className="space-y-1.5">
            {hermes.tasks.length === 0 ? (
              <div className="text-[11px] py-3 text-center" style={{ color: '#5E626A' }}>Keine Tasks generiert</div>
            ) : (
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px]" style={{ color: '#5E626A' }}>
              {hermes.lastRunAt ? `Letzter Lauf: ${new Date(hermes.lastRunAt).toLocaleTimeString('de-DE')}` : 'Noch nicht gestartet'}
            </span>
          </div>
          {hermes.nextRunAt && hermes.isRunning && (
            <span className="text-[10px]" style={{ color: '#5E626A' }}>
              Naechster: {new Date(hermes.nextRunAt).toLocaleTimeString('de-DE')}
            </span>
          )}
        </div>
      </div>
...
// TASK ROW COMPONENT
function TaskRow({ task, onConfirm, onCancel }: { task: HermesTask; onConfirm: (id: string) => void; onCancel: (id: string) => void; }) {
  ...
  return (
    <div className="rounded-lg p-2.5 space-y-1.5" style={{ backgroundColor: sc.bg, border: `1px solid ${sc.color}25` }}>
      <div className="flex items-center gap-2">
        <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: sc.color }} />
        <span className="text-[11px] font-medium flex-1 truncate" style={{ color: '#FFFFFF' }}>{task.title}</span>
        <span className="text-[9px] px-1 py-0.5 rounded shrink-0" style={{ backgroundColor: `${sc.color}20`, color: sc.color }}>
          {task.status === 'paused_awaiting_confirmation' ? 'Wartet' : task.status}
        </span>
      </div>
      <p className="text-[10px] line-clamp-2" style={{ color: '#A1A4AA' }}>{task.description}</p>
      {task.status === 'paused_awaiting_confirmation' && (
        <div className="flex items-center gap-1.5 pt-1">
          <button onClick={() => onConfirm(task.id)}
            ...
            <CheckCircle size={9} /> Bestaetigen
          </button>
          ...
          {task.legalRisk !== 'none' && (
            <span className="text-[9px] ml-auto" style={{ color: '#F5C542' }}>
              <AlertTriangle size={9} className="inline mr-0.5" /> Rechtlich
            </span>
          )}
        </div>
      )}
